...
knitr::include_graphics(file.path(tmpdir, "Correlogram.jpg"))
```
...
knitr::include_graphics(paste0(tmpdir, "/", basename(tmpdir), "-Index_Error_on_crossV.png"))
```
```{r, echo=FALSE, results='hide'}
# Save in inst
file.copy(paste0(tmpdir, "/", basename(tmpdir), "-Index_Error_on_crossV.png"),
          "Index_Error_on_crossV.png", overwrite = TRUE)
```
...
knitr::include_graphics(
  list.files(tmpdir, full.names = TRUE)[
    grep("Help_for_model_Choice_NbParam", list.files(tmpdir))]
  )
```
```{r, echo=FALSE, results='hide'}
# Save in inst
file.copy(list.files(tmpdir, full.names = TRUE)[
    grep("Help_for_model_Choice_NbParam", list.files(tmpdir))],
    "Help_for_model_Choice.png", overwrite = TRUE)
```
...
```{r, echo=FALSE, results='hide'}
# Save in inst
file.copy(paste0(tmpdir, "/", basename(tmpdir), "-Param_Exp_Deviance_", Num.Best, ".png"),
          "Param_Exp_Deviance.png", overwrite = TRUE)
```
...
# Save in inst
file.copy(
  paste0(tmpdir, "/", basename(tmpdir),
         "-CovForMeanPred_Marginals_", Num.Best, ".png"),
...
```{r, results='asis', echo=FALSE, purl=FALSE}
knitr::include_graphics(paste0(tmpdir, "/", basename(tmpdir), "-Thresholds_for_prediction_", Num.Best, ".png"))
```
```{r, echo=FALSE, results='hide'}
# Save in inst
file.copy(
  paste0(tmpdir, "/", basename(tmpdir),
         "-Thresholds_for_prediction_", Num.Best, ".png"),
  "Thresholds_for_prediction.png", overwrite = TRUE)
```
...
writeRaster(pred.r, filename = "predr.tif", overwrite = TRUE)
# Geotif does not save raster names
predrNames <- names(pred.r)
readr::write_rds(predrNames, path = "predrNames.rds")
```

The first output of interest is the first layer `resp.fit` of the `RasterStack`. Here, the predicted probability of presence.  
...
# To add in /inst for vignettes
readr::write_rds(BestThd, path = "BestThd.rds")
```
